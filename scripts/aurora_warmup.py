...
CREATE TABLE IF NOT EXISTS ragas_eval_runs (
  ...
  evaluated_at TIMESTAMPTZ DEFAULT NOW()
    ...
    ("ragas_evaluations.run_id", "ALTER TABLE ragas_evaluations ADD COLUMN IF NOT EXISTS run_id UUID"),
    ("ragas_evaluations.ground_truth", "ALTER TABLE ragas_evaluations ADD COLUMN IF NOT EXISTS ground_truth TEXT"),
    ("ragas_evaluations.hallucination_rate", "ALTER TABLE ragas_evaluations ADD COLUMN IF NOT EXISTS hallucination_rate NUMERIC(4,3)"),
    ("ragas_evaluations.contexts", "ALTER TABLE ragas_evaluations ADD COLUMN IF NOT EXISTS contexts JSONB DEFAULT '[]'"),
    ("ragas_evaluations.audit_json", "ALTER TABLE ragas_evaluations ADD COLUMN IF NOT EXISTS audit_json JSONB DEFAULT '{}'"),
    ("ragas_evaluations.run_id_idx", "CREATE INDEX IF NOT EXISTS ragas_evaluations_run_id_idx ON ragas_evaluations (run_id)"),
    ("ragas_eval_runs.at_idx", "CREATE INDEX IF NOT EXISTS ragas_eval_runs_evaluated_at_idx ON ragas_eval_runs (evaluated_at DESC)"),
    ("trading_eval_runs table", """
CREATE TABLE IF NOT EXISTS trading_eval_runs (
  ...
  evaluated_at TIMESTAMPTZ DEFAULT NOW()
    ...
    ("agent_performance.eval_run_id", "ALTER TABLE agent_performance ADD COLUMN IF NOT EXISTS eval_run_id UUID"),
    ("agent_performance.trade_id", "ALTER TABLE agent_performance ADD COLUMN IF NOT EXISTS trade_id UUID"),
    ("agent_performance.final_action", "ALTER TABLE agent_performance ADD COLUMN IF NOT EXISTS final_action VARCHAR(10)"),
    ("agent_performance.return_pct", "ALTER TABLE agent_performance ADD COLUMN IF NOT EXISTS return_pct NUMERIC(8,4)"),
    ("agent_performance.horizon_days", "ALTER TABLE agent_performance ADD COLUMN IF NOT EXISTS horizon_days INT DEFAULT 5"),
    ("agent_performance.audit_json", "ALTER TABLE agent_performance ADD COLUMN IF NOT EXISTS audit_json JSONB DEFAULT '{}'"),
    ("agent_performance.eval_run_idx", "CREATE INDEX IF NOT EXISTS agent_performance_eval_run_idx ON agent_performance (eval_run_id)"),
    ("simulated_trades.outcome_null_idx", "CREATE INDEX IF NOT EXISTS simulated_trades_outcome_null_idx ON simulated_trades (executed_at) WHERE outcome IS NULL"),
    ("trading_eval_runs.at_idx", "CREATE INDEX IF NOT EXISTS trading_eval_runs_evaluated_at_idx ON trading_eval_runs (evaluated_at DESC)"),
]
